...
def parse_args():
    ...
    p.add_argument(
        "--json-auto",
        action="store_true",
        help="自动生成 JSON 文件名（包含模型、后端、aten/onednn ISA、时间戳等信息）",
    )
    p.add_argument(
        "--json-dir",
        default=".",
...
def build_decoder_prefix(tokenizer, length: int):
    # Use Whisper's canonical start token sequence when available.
    base_prefix = list(getattr(tokenizer, "sot_sequence", []))
    if not base_prefix:
        base_prefix = [getattr(tokenizer, "sot", 0)]

    if args.without_timestamps:
        no_ts = getattr(tokenizer, "no_timestamps", None)
        if no_ts is not None and no_ts not in base_prefix:
            base_prefix.append(no_ts)

    if length < len(base_prefix):
        raise ValueError(
            f"decoder-tokens ({length}) must be >= required prefix length ({len(base_prefix)})"
        )
    ...
    if not extra_ids:
        # Keep the benchmark deterministic even if tokenizer.encode fails.
        fallback = getattr(tokenizer, "eot", base_prefix[-1] if base_prefix else 0)
        extra_ids = [fallback]

    # Repeat a stable token pattern so each run uses the same prefix length.
    out = base_prefix[:]
...
def main():
    ...
    if args.json_out:
        ...
        # 自动生成文件名：whisper_bench_{model}_{backend}_{aten_isa}_{onednn_isa}_{timestamp}.json
