...
# oneDNN ISA
if args.onednn_isa != "auto":#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def parse_args():
    ...
    p.add_argument(
        "--sections",
        default="encoder,decoder_prefill,decoder_cached_1tok,full",
        help="要 benchmark 的部分：encoder,decoder_prefill,decoder_cached_1tok,full，逗号分隔",
    )
    p.add_argument("--warmup", type=int, default=3, help="正式计时前 warmup 次数")
    p.add_argument("--repeat", type=int, default=5, help="正式计时次数")
    p.add_argument("--decoder-tokens", type=int, default=16, help="decoder 前缀 token 长度")
    p.add_argument(
        "--without-timestamps",
        action="store_true",
        help="full transcribe 时使用 without_timestamps=True",
    ...
    p.add_argument(
        "--profile-sections",
        default="",
        help="要做 profiler 的部分：encoder,decoder_prefill,decoder_cached_1tok,full；空表示不做",
    )
    p.add_argument("--profile-topk", type=int, default=30, help="profiler 表显示前多少行")
    p.add_argument(
        "--profile-sort-by",
...
def decoder_prefill_once(model, decoder_tokens, audio_features):
    logits = model.decoder(decoder_tokens, audio_features)
    next_token = logits[:, -1, :].argmax(dim=-1)
    return logits, next_token


def decoder_cached_1tok_once(model, decoder_tokens, audio_features):
    """
    更接近真实增量解码：
    1. 先用 prefix[:-1] 建 KV cache
    2. 再只输入最后 1 个 token
    """
    kv_cache, hooks = model.install_kv_cache_hooks()
    try:
        if decoder_tokens.shape[1] > 1:
            _ = model.decoder(decoder_tokens[:, :-1], audio_features, kv_cache=kv_cache)
            step_in = decoder_tokens[:, -1:]
        else:
            step_in = decoder_tokens

        logits = model.decoder(step_in, audio_features, kv_cache=kv_cache)
        next_token = logits[:, -1, :].argmax(dim=-1)
        return logits, next_token
    finally:
        remove_hooks(hooks)
...
def main():
    ...
    valid_sections = {"encoder", "decoder_prefill", "decoder_cached_1tok", "full"}
    ...
    # encoder benchmark：固定 30s chunk
    ...
    # 先准备 audio_features，供 decoder benchmark 使用
    need_decoder = ("decoder_prefill" in sections) or ("decoder_cached_1tok" in sections)
    audio_features = None
    if need_decoder:
        with torch.inference_mode():
    ...
    # ---------------- decoder_prefill ----------------
    if "decoder_prefill" in sections:
        def run_decoder_prefill():
            return decoder_prefill_once(model, decoder_tokens, audio_features)

        prefill_times, prefill_out = time_fn(run_decoder_prefill, args.warmup, args.repeat)
        logits, next_token = prefill_out
        ...
        print_section_summary(
            ...
            extra={
                ...
                "steps_per_sec(mean)": f"{1.0 / statistics.mean(prefill_times):.3f}",
                "next_token": next_token.tolist(),
            },
        )

        results["bench"]["decoder_prefill"] = {
            "times_sec": prefill_times,
            "prefix_len": int(decoder_tokens.shape[1]),
            "logits_shape": tuple(logits.shape),
            "steps_per_sec_mean": 1.0 / statistics.mean(prefill_times),
            "next_token": next_token.tolist(),
        }

        if "decoder_prefill" in profile_sections:
            _, table = run_profile_once("decoder_prefill", run_decoder_prefill)
            results["bench"]["decoder_prefill"]["profiler_table"] = table

    # ---------------- decoder_cached_1tok ----------------
    if "decoder_cached_1tok" in sections:
        def run_decoder_cached_1tok():
            return decoder_cached_1tok_once(model, decoder_tokens, audio_features)

        cached_times, cached_out = time_fn(run_decoder_cached_1tok, args.warmup, args.repeat)
        logits, next_token = cached_out
        ...
        print_section_summary(
            ...
            extra={
                ...
                "step_input_len": 1,
                "logits_shape": tuple(logits.shape),
                "steps_per_sec(mean)": f"{1.0 / statistics.mean(cached_times):.3f}",
                "next_token": next_token.tolist(),
            },
        )

        results["bench"]["decoder_cached_1tok"] = {
            "times_sec": cached_times,
            "prefix_len": int(decoder_tokens.shape[1]),
            "step_input_len": 1,
            "logits_shape": tuple(logits.shape),
            "steps_per_sec_mean": 1.0 / statistics.mean(cached_times),
            "next_token": next_token.tolist(),
        }

        if "decoder_cached_1tok" in profile_sections:
            _, table = run_profile_once("decoder_cached_1tok", run_decoder_cached_1tok)
            results["bench"]["decoder_cached_1tok"]["profiler_table"] = table
